...
(function() {
  ...
  http.createServer(function(req, res) {
    ...
    blnk = function(str, pfx, sfx, replSpc) {
      if (pfx == null) {
        pfx = '';
      }
      if (sfx == null) {
      ...
      if (str) {
        str = str.replace(/\n|\s+/g, ' ').replace(/^\s*/g, '').replace(/\s*$/g, '').replace(/\s/g, replSpc);
        if (str) {
          return pfx + str + sfx;
        } else {
          return '';
    ...
    if (req.url === '/forecast') {
      $.json(forecastURL, function() {});
      request(forecastURL, function(err, resp, twcHtml) {
        var $cont, $parts, $section, chanceRain, dump, hiParts, high, humidity, iconURL, indent, phrase, uv, wind;
        indent = '';
        dump = function(ele) {
          indent += '  ';
          if (ele.tagName === 'tr') {
          ...
          console.log(indent, ele.tagName, blnk($(ele).attr('id'), '#'), blnk($(ele).attr('class'), '.', '', '.'), blnk(text, '"', '"'), blnk($(ele).attr('src'), 'src: '));
