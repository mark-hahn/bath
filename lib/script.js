...
(function() {
  var dateMS, div, dowColor, flash, img, large, lastHour, medium, raw, refreshCurAndTime, refreshFore, refreshSize, render, small;
  ...
  (refreshSize = function() {
    var $hdr, bodyWid, hdrId, _i, _j, _len, _len1, _ref, _ref1, _results;
    bodyWid = $('body').width();
    large = (bodyWid / 4) + 'px';
    medium = (bodyWid / 6) + 'px';
    small = (bodyWid / 18) + 'px';
    ...
    _ref1 = ['rain', 'humidity', 'uv', 'phrase'];
    _results = [];
    for (_j = 0, _len1 = _ref1.length; _j < _len1; _j++) {
      hdrId = _ref1[_j];
      _results.push($hdr = $('#' + hdrId).css({
        fontSize: small
      }));
    }
    return _results;
  ...
  refreshFore = function() {
    return $.getJSON('/forecast', function(data) {
      var chanceRain, high, humidity, iconURL, phrase, uv, wind;
      iconURL = data.iconURL, high = data.high, phrase = data.phrase, chanceRain = data.chanceRain, wind = data.wind, humidity = data.humidity, uv = data.uv;
      return $('#forecast').replaceWith(render(function() {
        return div('#forecast', {
          ...
          div('#row1', {
            ...
            div({
              style: 'clear:both; float:left; margin-left:5%; width:45%; height:100%'
            }, function() {
              return img({
            ...
            return div({
              style: 'float:right; text-align:right; color:white; width:40%; margin-right:10%;'
            }, function() {
          ...
          div('#row2', {
            style: 'clear:both; float:left; margin-top:0%; color:white; width:100%; height:20%'
          }, function() {
            div({
              style: 'clear:both; float:left; margin-left:10%; width:45%'
            }, function() {
              return div('#phrase', (phrase ? phrase : ''));
            });
            return div({
              style: 'float:right; margin-right:10%; width:35%; text-align:right'
            }, function() {
              return div('#humidity', (humidity ? 'Hum ' + humidity : ''));
            });
          });
          return div('#row3', {
            style: 'clear:both; float:left;  margin-top:3%; color:white; width:100%; height:15%'
          }, function() {
            div({
              style: 'clear:both; float:left; margin-left:10%; width:48%'
            }, function() {
              return div('#uv', (uv ? 'UV ' + uv.replace(/[-\s]+/g, ' ') : ''));
            });
            return div({
              style: 'float:right; margin-right:10%; width:32%; text-align:right'
            }, function() {
              return div('#rain', (chanceRain ? 'Rain ' + chanceRain : ''));
            });
          });
        });
      }));
  ...
  refreshCurAndTime = function() {
    ...
    $.get('/cumulus/realtime.txt', function(data) {
      ...
      return $('#current').replaceWith(render(function() {
        return div('#current', {
          style: 'clear:both; float:left; position:relative; top:-7%; width:100%; height:36%'
        }, function() {
          div({
          ...
          return div({
            style: 'clear:both; float:left; margin:0% 0 0% 12%; color:white; font-size:' + medium
          }, function() {
            return raw(avgWind + '&nbsp;-&nbsp;' + gust + '&nbsp;&nbsp;mph');
          });
        });
      }));
    });
